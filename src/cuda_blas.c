...
extern void dot_fwd_cublas(float *act, float *weights, float *zout, int Batch, int In, int Out);
extern void dot_bwd_cublas(float *deltas, float *weights, float *bwdout, int Batch, int In, int Out);
extern void dot_grd_cublas(float *act, float *deltas, float *grd, int Batch, int In, int Out);
...
void setup_layer(struct layer *layer, int Batch, int In, int Out) {
	...
	//Kaiming initialization scheme for weights
...
void forward_pass(struct layer *ll, int lastlayer) {
	...
	dot_fwd_cublas((void *)ll->a_list, (void *)ll->weights, 
...
void bwd_pass(struct layer *ll, int lastlayer) {
	...
	dot_bwd_cublas((void *)ll->deltas, (void *)ll->weights, (void *)ll->bwd, Batch, In, Out);
...
void weightupd(struct layer *ll, int num, int step) {
	...
	dot_grd_cublas(act, del, grd, Batch, In, Out);
...
int main(const int argc, const char** argv){
	...
	int numTrainingImgs = 50000;  // Number of training images
	int numValidationImgs = 10000;  // Number of validation images

	// Split the training data
	float *trainingImages = images;  // First 50K images for training
	float *validationImages = images + numTrainingImgs * inputSize;  // Last 10K images for validation

	float *trainingLabels = labels;  // First 50K labels for training
	float *validationLabels = labels + numTrainingImgs * outputSize;  // Last 10K labels for validation
	...
	for (int epoch = 0; epoch < ne; ++epoch) {
		clock_t start = clock();
		// Train using the 50K training images
		int match = train(nn, numTrainingImgs / nb, numTrainingImgs, trainingImages, trainingLabels);
		float trainAcc = ((float)match) / numTrainingImgs * 100;
		printf("Loss: %d\n", match);
		clock_t end = clock();

		// Evaluate on the validation set
		int valMatch = test(nn, numValidationImgs/nb, numValidationImgs, validationImages, validationLabels);
		float valAcc = ((float)valMatch) / numValidationImgs * 100;

		// Output training and validation accuracy
		printf("Epoch %d: Training accuracy: %2.2f%%, Validation accuracy: %2.2f%%\n", epoch, trainAcc, valAcc);
